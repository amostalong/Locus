...
describe("reduceStreamEvent", () => {
  ...
  describe("askUser", () => {
    ...
    it("queues multiple askUser events from the same LLM response (does not overwrite)", () => {
      ...
      // meant that 3 parallel `ask_user_question` tool calls from one
      // LLM response only ever showed the last one in the UI, and the
      // other oneshot receivers in the backend were never resolved.
      const baseState = () => makeState({ isStreaming: true });
      ...
      const apply = (state: StreamState, event: StreamEvent) => {
        for (const m of reduceStreamEvent(state, event)) {
          applyTestMutation(state, m);
        }
      };

      const state = baseState();
      apply(state, mkAsk("q1", "Pick a name?"));
      apply(state, mkAsk("q2", "Pick a license?"));
      apply(state, mkAsk("q3", "Publish now?"));

      expect(state.pendingQuestions).toHaveLength(3);
      expect(state.pendingQuestions.map((q) => q.questionId)).toEqual(["q1", "q2", "q3"]);
      // The first question is the head of the queue (what the UI shows).
    ...
    it("de-duplicates enqueueQuestion by questionId", () => {
      // Safety net: if the LLM or backend ever replays the same askUser
      // event (e.g. on session restore), the queue should not grow.
      const state = makeState({ isStreaming: true });
      const event: StreamEvent = {
        runId: "test-run",
        type: "askUser",
