...
const sessionServiceMocks = vi.hoisted(() => ({
  ...
  getSessionPlanState: vi.fn().mockResolvedValue({ active: false }),
  ...
  setSessionPlanMode: vi.fn().mockResolvedValue({ active: false }),
...
const modelStoreMocks = vi.hoisted(() => ({
  ...
  applySessionEffort: vi.fn(),
  setActiveSessionId: vi.fn(),
  hydrateSessionOverrides: vi.fn(),
  hydrateSessionEfforts: vi.fn(),
  selectSessionModel: vi.fn(),
  selectSessionEffort: vi.fn(),
...
describe("chat session panel state", () => {
  ...
  it("restores the last sent model for an existing session and resets new chats to the default", async () => {
    ...
    // Fork (per-session override design): switching sessions hydrates the
    // session-scoped override map instead of mutating the global selection.
    // The backend maps the persisted last_model_id into `modelId`, so a
    // null `modelId` releases the override (session follows the global).
    expect(modelStoreMocks.applySessionModel).toHaveBeenCalledWith("s1", null);
    expect(modelStoreMocks.setActiveSessionId).toHaveBeenCalledWith("s1");
