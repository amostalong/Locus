...
/// Decode raw file bytes to String + had_bom flag.
/// Handles UTF-8 (with/without BOM), UTF-16 LE/BE (with BOM), and falls back
/// to lossy UTF-8 for files with a few invalid bytes (e.g. Unity YAML with
/// embedded binary references).
fn decode_file_bytes(bytes: Vec<u8>) -> (String, bool) {
    if bytes.starts_with(&UTF8_BOM) {
        return (String::from_utf8_lossy(&bytes[UTF8_BOM.len()..]).into_owned(), true);
    }
    if bytes.starts_with(&UTF16_LE_BOM) {
        let words: Vec<u16> = bytes[UTF16_LE_BOM.len()..]
            .chunks_exact(2)
            .map(|c| u16::from_le_bytes([c[0], c[1]]))
            .collect();
        return (String::from_utf16_lossy(&words), false);
    }
    if bytes.starts_with(&UTF16_BE_BOM) {
        let words: Vec<u16> = bytes[UTF16_BE_BOM.len()..]
            .chunks_exact(2)
            .map(|c| u16::from_be_bytes([c[0], c[1]]))
            .collect();
        return (String::from_utf16_lossy(&words), false);
    }
    // Plain UTF-8; use lossy conversion so files with a handful of invalid
    // bytes (e.g. some Unity YAML assets) open instead of returning an error.
